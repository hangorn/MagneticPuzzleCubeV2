...
function Cube(mats, cubeSize, imgs, sectionImgs, numberOfSections) {
	...
	// Array de objetos Face, que contendrá los 6 objetos que representan las 6 caras del cubo, donde se guardará
	 ...
	 ******************************************************************************************************************/
	/**
	 * Constructor de la clase Cube
	 * 
	 * @param Material:mats
	 *            array con todas las imágenes (materiales) que formarán el puzzle, de las cuales se seleccionarán 6.
	 * @param Float:cubeSize
	 *            tamaño del cubo en píxeles.
	 * @param Integer[]:imgs
	 *            array con los indices de los 6 materiales de cada cara del cubo, siguiendo el orden descrito arriba.
	 * @param Vector3[]:sectionImgs
	 *            Array con 6 vectores de 3 elementos. En X e Y 0,0 1,0 2,0 guardaremos la sección (0,0 para la sección
	 *            arriba a la 0,1 1,1 2,1 izquierda, 2,2 para la sección de abajo a la derecha) y 0,2 1,2 2,2 en Z la
	 *            rotación de la imagen en grados.
	 * @param Vector3:pos
	 *            vector de 3 elementos con la posición inicial del puzzle.
	 * @param Integer:numberOfSections
	 *            número de secciones en los que se dibidirá cada imagen de una cara.
	 */
	...
	for (var i = 0; i < 6; i++) {
		...
		// Obtenemos las secciones correspondiente a la la imagen inversa (ya que en la ultima version de THREE.JS se
		...
		// 1
		coords.push(new THREE.Vector2(unTercio * (sectionImgs[i].x), unTercio
				* ((numberOfSections - 1) - sectionImgs[i].y) + unTercio));
		// 0
		coords.push(new THREE.Vector2(unTercio * (sectionImgs[i].x), unTercio
				* ((numberOfSections - 1) - sectionImgs[i].y)));
		// 3
		coords.push(new THREE.Vector2(unTercio * (sectionImgs[i].x) + unTercio, unTercio
				* ((numberOfSections - 1) - sectionImgs[i].y)));
		// 2
		coords.push(new THREE.Vector2(unTercio * (sectionImgs[i].x) + unTercio, unTercio
				* ((numberOfSections - 1) - sectionImgs[i].y) + unTercio));
	...
	// Guardamos la informacion de cada cara: imagen, seccion de la imagen y angulo de rotacion en un nuevo objeto de la
	// clase Face
	for (var i = 0; i < 6; i++) {
		faces[i] = new Face(i, imgs[i], sectionImgs[i].x, sectionImgs[i].y, sectionImgs[i].z - 360
				* Math.floor(sectionImgs[i].z / 360));
	}
	...
	/**
	 * Método que devuelve el objeto Face con el indice indicado
	 * 
	 * @param Integer:index
