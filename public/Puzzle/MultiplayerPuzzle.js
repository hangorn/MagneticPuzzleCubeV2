...
function MultiplayerPuzzle(numC, mats) {
	 ...
	 ******************************************************************************************************************/
	/**
	 * Constructor de la clase MultiplayerPuzzle
	 * 
	 * @param Integer:numC
	...
	var positionPuzzle = function(object) {
		...
		for (var i = 0; i < group.children.length; i++) {
			...
			for (var k = 0; k < 6; k++) {
				...
				var pos = new THREE.Vector3(group.children[i].position.x, group.children[i].position.y,
	...
	/**
	 * Método para saber la rotación mas cercana a un angulo de 90º local al puzzle (group) de la figura proporcionada
	 * 
	 * @param Object3D:object
	...
	var getCenterCube = function() {
		...
		for (var i = 0; i < group.children.length; i++) {
			...
			for (var j = 0; j < group.children.length; j++) {
				...
				if (Math.abs(group.children[i].position.x - group.children[j].position.x) > dist
						|| Math.abs(group.children[i].position.y - group.children[j].position.y) > dist
						|| Math.abs(group.children[i].position.z - group.children[j].position.z) > dist) {
					return null;
				}
...
var getCube = function(pos) {
	for (var i = 0; i < group.children.length; i++) {
		if (group.children[i].position.x == pos.x && group.children[i].position.y == pos.y
				&& group.children[i].position.z == pos.z) {
			return group.children[i];
		}
	}
	return null;
...
var getFaceCubes = function(face, center) {
	...
	if (numberOfCubes == 3) {
		...
		case 1:
			...
			for (var i = 1; i > -2; i--) {
				for (var j = 1; j > -2; j--) {
					cubes.push(getCube(new THREE.Vector3(cubeSize + center.x, cubeSize * i + center.y, cubeSize * j
							+ center.z)));
				}
			}
			break;
		...
		case 3:
			...
			for (var i = -1; i < 2; i++) {
				for (var j = -1; j < 2; j++) {
					cubes.push(getCube(new THREE.Vector3(cubeSize * j + center.x, cubeSize + center.y, cubeSize * i
							+ center.z)));
				}
			}
			break;
		...
		case 5:
			...
			for (var i = -1; i < 2; i++) {
				for (var j = 1; j > -2; j--) {
					cubes.push(getCube(new THREE.Vector3(cubeSize * j + center.x, cubeSize * i + center.y, cubeSize
							+ center.z)));
				}
			}
			break;
	...
	if (numberOfCubes == 2) {
		...
		case 1:
			...
			for (var i = 1; i > -2; i -= 2) {
				for (var j = 1; j > -2; j -= 2) {
					cubes.push(getCube(new THREE.Vector3(cubeSize / 2 + center.x, cubeSize / 2 * i + center.y, cubeSize
							/ 2 * j + center.z)));
				}
			}
			break;
		...
		case 2:
			...
			for (var i = 1; i > -2; i -= 2) {
				for (var j = -1; j < 2; j += 2) {
					cubes.push(getCube(new THREE.Vector3(-cubeSize / 2 + center.x, cubeSize / 2 * i + center.y,
							cubeSize / 2 * j + center.z)));
				}
			}
			break;
		// Cara superior
		case 3:
			...
			for (var i = -1; i < 2; i += 2) {
				for (var j = -1; j < 2; j += 2) {
					cubes.push(getCube(new THREE.Vector3(cubeSize / 2 * j + center.x, cubeSize / 2 + center.y, cubeSize
							/ 2 * i + center.z)));
				}
			}
			break;
		...
		case 4:
			...
			for (var i = 1; i > -2; i -= 2) {
				for (var j = -1; j < 2; j += 2) {
					cubes.push(getCube(new THREE.Vector3(cubeSize / 2 * j + center.x, -cubeSize / 2 + center.y,
							cubeSize / 2 * i + center.z)));
				}
			}
			break;
		// Cara delantera
		case 5:
			...
			for (var i = -1; i < 2; i += 2) {
				for (var j = 1; j > -2; j -= 2) {
					cubes.push(getCube(new THREE.Vector3(cubeSize / 2 * j + center.x, cubeSize / 2 * i + center.y,
							cubeSize / 2 + center.z)));
				}
			}
			break;
 ...
 *            6=trasera.
 * @return Vector2 vector de dos componentes donde el primero es el identificador de la cara y el segundo la rotacion de
 *         esta.
 */
var getFaceIndex = function(cube, puzzleFace) {
	...
	// Obtenemos las coordenadas de la cara que estamos buscando y aprovechamos que el cubo ya las tiene creadas
	// para extraerselas
	var faceCoords = [];
...
/**
 * Método para averiguar si el último cubo introducido en el puzzle esta bien colocado, es decir, si alguna de las caras
 * del cubo esta bien colocada respecto a algun cubo que ya esté en el puzzle.
 * 
 * @return Boolean booleano que será true si alguna cara del último cubo colocado coincide con alguna del puzzle, false
 *         si no.
 */
