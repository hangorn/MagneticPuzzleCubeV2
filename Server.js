...
Server = function() {
	 ...
	 **************************************************************************/
	/*
	 * Nombre: Constructor Sinopsis: Constructor de la clase Server. Entradas:
	 * Salidas:
	 */
	...
	/*
	 * Nombre: solvedGame Sinopsis: Método que se ejecutará al resolver el
	 * puzzle. Entradas: -Game:game -> objeto de la clase Game con los datos de
	 * la partida. Salidas:
	 */
	...
	/*
	 * Nombre: saveLastScore Sinopsis: Método para guardar una puntuación cuando
	 * empieze otra partida o se desconecte el cliente, si hay alguna puntuacion
	 * que guardar. Entradas: -Client:client-> objeto de la clase cliente con
	 * los datos del cliente que quiere desconectarse. Salidas:
	 */
	this.saveLastScore = function(client) {
		...
		if (client.lastPartner != undefined && client.lastPartner.savingScore) {
			...
			if (score.submode == 2 || score.submode == 3) {
				// Si el cliente que quiere guardar la puntuacion es el ganador
				// lo
			...
			dao.saveScore(score);
			// Marcamos a ambos jugadores como que no quieren guardar la
			// puntuacion
	...
	/*
	 * Nombre: createGame Sinopsis: Método para crear una partida en el
	 * servidor. Entradas: -Client:client-> objeto de la clase cliente con los
	 * datos del cliente que ha creado la partida. -String:name -> cadena con el
	 * nombre de la partida. -Integer:type -> entero que identificará el tipo de
	 * partida. -String[]:images -> array con cadenas de texto que representan
	 * las imágenes con las que se ha iniciado la partida codificadas en base
	 * 64. -Vector3[]:iniPos -> posiciones iniciales de los cubos de la partida
	 * creada. -Vector3[]:iniRot -> rotaciones iniciales de los cubos de la
	 * partida creada. Salidas:
	 */
	this.createGame = function(client, name, type, images, iniPos, iniRot) {
		...
		console.log("Game named: " + name + " type: " + type
				+ " was created with ID: " + game.ID + " by the player "
				+ game.players.host.ID + " with " + game.images.length
				+ " images.  We have " + games + " games");
	}

	/*
	 * Nombre: sendGames Sinopsis: Método para enviar al cliente la información
	 * de todas las partidas disponibles. Entradas: -Client:client-> objeto de
	 * la clase cliente con los datos del cliente que quiere recibir las
	 * partidas disponibles. Salidas:
	 */
	...
	/*
	 * Nombre: joinClient Sinopsis: Método para unir un cliente a una partida
	 * disponible. Entradas: -Client:client-> objeto de la clase cliente con los
	 * datos del cliente que se quiere unir a la partida. -String:gameID -> ID
	 * del juego al que se quiere unir el cliente. Salidas:
	 */
	...
	/*
	 * Nombre: readyToPlay Sinopsis: Método para que un cliente se una a una
	 * partida disponible. Entradas: -Client:client-> objeto de la clase cliente
	 * con los datos del cliente que esta listo para jugar. Salidas:
	 */
	this.readyToPlay = function(client) {
		...
		client.ready = true;
		// Si el cliente es el host de la partida y el cliente de la partida ya
		// esta listo
		if (game.players.host == client && game.players.client.ready) {
			// Indicamos a ambos jugadores que los dos estan listos para empezar
			// la partida
			client.socket.emit('onAllReady', {});
			game.players.client.socket.emit('onAllReady', {});
			// Miramos a ver si alguno de los dos jugadores tiene alguna
			// puntuacion pendiente de guardar
		...
		// Si el cliente es el cliente de la partida y el host de la partida ya
		// esta listo
	...
	/*
	 * Nombre: finishGame Sinopsis: Método para terminar una partida en el
	 * servidor. Entradas: -Client:client-> objeto de la clase cliente con los
	 * datos del cliente que quiere borrar la partida. -String:gameID -> ID del
	 * juego que será terminado. Salidas:
	 */
	this.finishGame = function(client, gameID) {
		...
		if (game.players.client != null && game.players.host != null) {
			...
			if (client == game.players.host) {
				// Le decimos al cliente de la partida que se ha desconectado el
				// host
			...
			if (client == game.players.client) {
				// Le decimos al host de la partida que se ha desconectado el
				// cliente
	...
	/*
	 * Nombre: sendScores Sinopsis: Método para enviar al cliente la información
	 * de las puntuaciones del modo solicitado. Entradas: -Client:client->
	 * objeto de la clase cliente con los datos del cliente que quiere recibir
	 * las puntuaciones. -Integer:mode -> modo del que se quieren obtener las
	 * puntuaciones. -Integer:submode -> submodo del que se quieren obtener las
	 * puntuaciones. Salidas:
	 */
	...
	/*
	 * Nombre: saveScore Sinopsis: Método para guardar una puntuación. Entradas:
	 * -Client:client-> objeto de la clase cliente con los datos del cliente que
	 * quiere guardar una puntuación. -Data:data-> objeto que contendrá los
	 * datos de la puntuación a guardar : nombre, puntuación, fecha, modo y
	 * submodo. Salidas:
	 */
	this.saveScore = function(client, data) {
		...
		// Si es un modo multijugador
			...
			client.score = data;
			// Si el ultimo compañero con el que se jugo ya quiere guardar la
			// puntuacion
			if (client.lastPartner != undefined
					&& client.lastPartner.savingScore == true) {
				var score = client.score;
				// Guardamos los nombres de los jugadores
				// Si se trata del modo contrareloj
				if (score.submode == 2 || score.submode == 3) {
					// Si el cliente que quiere guardar la puntuacion es el
					// ganador lo
					// ponemos primero, si no ponemos primero al otro jugador
					if (client.winner)
						score.name = client.score.name + '#'
								+ client.lastPartner.score.name;
					else
						score.name = client.lastPartner.score.name + '#'
								+ client.score.name;
				}
				// Si se trata del modo cooperativo
				else
